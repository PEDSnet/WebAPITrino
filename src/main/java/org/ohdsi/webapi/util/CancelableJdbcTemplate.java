...
public class CancelableJdbcTemplate extends JdbcTemplate {
  ...
  private boolean supportsBatchUpdates(Connection connection) throws SQLException {
    ...
    return JdbcUtils.supportsBatchUpdates(connection) && !connection.getMetaData().getURL().startsWith("jdbc:impala") && !connection.getMetaData().getURL().startsWith("jdbc:trino");
  }
}
